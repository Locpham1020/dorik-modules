        ...
        window.DorikUtils?.error('❌ Container processing error:', error);
      } finally {
    ...
    async processBatch(containers) {
      ...
      try {
        window.DorikUtils?.log(`📦 Processing batch: ${containerIds.length} containers`);
        ...
        window.DorikUtils?.log(`✅ Batch completed: ${containerIds.length} containers processed`);

      } catch (error) {
        window.DorikUtils?.error('❌ Batch processing failed:', error);
      }
    },
    ...
    setupMemoryMonitoring() {
      setInterval(() => {
        ...
        if (memoryInfo && memoryInfo.used_mb > 100) { // If using more than 100MB
          window.DorikUtils?.log(`⚠️ High memory usage: ${memoryInfo.used_mb}MB`);
    ...
    async loadContainer(containerId) {
      ...
      if (window.DorikState?.processedContainers.has(containerId)) {
        window.DorikUtils?.log(`📦 Container ${containerId} already processed`);
        return true;
      }

      try {
        window.DorikUtils?.log(`🔄 Manually loading container: ${containerId}`);
        ...
        window.DorikUtils?.log(`✅ Container ${containerId} loaded manually`);
        return true;

      } catch (error) {
        window.DorikUtils?.error(`❌ Manual loading failed for ${containerId}:`, error);
        return false;
      }
    },
    ...
    async loadVisibleContainers() {
      ...
      if (visibleContainers.length > 0) {
        window.DorikUtils?.log(`🎯 Loading ${visibleContainers.length} visible containers`);
    ...
    async preloadNearbyContainers(distance = 300) {
      ...
      if (nearbyContainers.length > 0) {
        ...
        nearbyContainers.sort((a, b) => a.dist/**
...
(function() {
  ...
  window.DorikLazy = {
    ...
    init() {
      window.DorikUtils?.log('👁️ Initializing lazy loading system...');
      ...
      window.DorikUtils?.log('✅ Lazy loading system ready');
    },
    ...
    createObserver() {
      ...
      window.DorikUtils?.log('👁️ Intersection observer created');
    },
    ...
    observeContainers() {
      ...
      window.DorikUtils?.log(`👁️ Observing ${containers.size} containers`);
    ...
    async processPendingContainers() {
      ...
      try {
        ...
        window.DorikUtils?.log(`📦 Processing ${this.containersToLoad.length} pending containers`);
